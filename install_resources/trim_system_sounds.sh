...
MIN_SILENCE_DURATION=0.5	# Minimum duration to consider silence
...
trim_file() {
	...
	local last_silence_end
	last_silence_end=$(ffmpeg -i "$infile" -af "silencedetect=noise=$SILENCE_NOISE_LEVEL:d=$MIN_SILENCE_DURATION" \
						-f null - 2>&1 | grep "silence_end" | tail -n 1 | awk '{print $5}')

	if [[ -n "$last_silence_end" ]]; then
		local new_duration
		new_duration=$(echo "$last_silence_end + $EXTRA_SILENCE" | bc -l)
