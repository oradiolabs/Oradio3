...
STRESS_RESTART_THRESHOLDS = {
    "avg_per_action": 0.25,         # seconds/action (allowing reconnect delays)
    "max_per_action": 2.50,         # seconds/action
    "errors": 0,                    # no exceptions should escape _execute()
    "retries_max": 30,              # tolerate some retries while reconnecting
    "min_reconnects": 1,            # at least 1 reconnect logged in window
}
...
class MPDTester:
    ...
    @staticmethod
    def count_connections(logfile=ORADIO_LOG, start_time=None, end_time=None):
        """
        Count 'Connected to MPD on' entries within a time window.
        """
        if start_time is None:
            return 0
        try:
        ...
        for line in lines:
            ...
            if end_time is not None and ts > end_time:
                continue
            if "Connected to MPD on " in line:
                found += 1
        return found
...
# ---------- MPD restart helpers ----------
def restart_mpd_robust():
    """
    Try to restart MPD using several methods.
    Returns (ok: bool, msg: str).
    """
    cmds = []
    if os.geteuid() == 0:
        cmds.append(["systemctl", "restart", "mpd"])
        cmds.append(["service", "mpd", "restart"])
    else:
        cmds.append(["sudo", "-n", "systemctl", "restart", "mpd"])
        cmds.append(["sudo", "-n", "service", "mpd", "restart"])
        cmds.append(["systemctl", "--user", "restart", "mpd"])  # last resort; usually no-op for system MPD
    ...
    for cmd in cmds:
        try:
            ...
            last_err = f"{' '.join(cmd)} -> {rc.returncode} | {rc.stderr.strip()}"
        except Exception as e:  # pylint: disable=broad-except
            last_err = f"{' '.join(cmd)} EXC: {e}"
...
def stress_test_with_mpd_restarts(client, iterations=120, delay=0.02, restarts=2, interval=2.0):
    """
    Sequential rapid commands while restarting MPD 'restarts' times at 'interval' seconds.
    Returns metrics dict + restart outcomes.
    """
    actions = [
        client.play,
        client.pause,
    ...
    restart_results = {"attempts": 0, "success": 0, "errors": []}

    def restart_thread():
        for k in range(restarts):
            if restart_stop.wait(interval):
                return
            ok, msg = restart_mpd_robust()
            restart_results["attempts"] += 1
            if ok:
                restart_results["success"] += 1
            else:
                restart_results["errors"].append(msg)

    t_restart = threading.Thread(target=restart_thread, daemon=True)
    ...
    for i in range(iterations):
        ...
        try:
            ...
            errors += 1
            oradio_log.error("Restart-stress action error at i=%d: %s", i, e)
        t1 = time.perf_counter()
        per_action_times.append(t1 - t0)
        if delay:
            time.sleep(delay)
    restart_stop.set()
    t_restart.join(timeout=5.0)
    total = time.time() - start

    return {
        "iterations": iterations,
        "restarts_requested": restarts,
        "restart_attempts": restart_results["attempts"],
        "restart_success": restart_results["success"],
        "restart_errors": restart_results["errors"],
        "total": total,
        "avg": (sum(per_action_times) / len(per_action_times)) if per_action_times else 0.0,
        "min": min(per_action_times) if per_action_times else 0.0,
        "max": max(per_action_times) if per_action_times else 0.0,
...
def run_all_tests():
    ...
    # -------- Stress Test with MPD Restarts --------
    print("\nRunning stress test with MPD restarts...")
    rs_start = time.time()
    rs = stress_test_with_mpd_restarts(
        tester.client,
        iterations=120,   # little longer to allow multiple restarts
        delay=0.02,
        restarts=2,       # number of restarts to trigger
        interval=2.0      # seconds between restarts
    )
    rs_end = time.time()
    rs_retries = tester.check_log_for_retries(
        logfile=ORADIO_LOG, start_time=rs_start, end_time=rs_end
    )
    rs_reconnects = tester.count_connections(
        logfile=ORADIO_LOG, start_time=rs_start, end_time=rs_end
    )

    print("\n--- STRESS TEST REPORT (Sequential + MPD Restarts) ---")
    print(f"Requested restarts: {rs['restarts_requested']}  |  Attempts: {rs['restart_attempts']}  |  Success: {rs['restart_success']}")
    if rs['restart_errors']:
        print("Restart errors:")
        for e in rs['restart_errors']:
            print(f"  - {e}")
    print(f"Actions: {rs['iterations']}")
    print(f"Total time: {rs['total']:.2f}s")
    print(f"Avg per action: {rs['avg']:.3f}s  |  Min: {rs['min']:.3f}s  |  Max: {rs['max']:.3f}s  |  P95: {rs['p95']:.3f}s")
    print(f"Action errors: {rs['errors']}")
    print(f"Retry count in window: {rs_retries}")
    print(f"Reconnect entries in log window: {rs_reconnects}")

    print("\n--- KPI EVALUATION (Stress + MPD Restarts) ---")
    def kpi_line_rs(label, value, limit):
        ok = value <= limit
        sym = "✅" if ok else "❌"
        print(f"{sym} {label}: {value:.3f}s (target ≤ {limit}s)")
        return ok
    ...
    rs_ok = True
    rs_ok &= kpi_line_rs("Avg per action", rs["avg"], STRESS_RESTART_THRESHOLDS["avg_per_action"])
    rs_ok &= kpi_line_rs("Max per action", rs["max"], STRESS_RESTART_THRESHOLDS["max_per_action"])
    ok_err_rs = (rs["errors"] <= STRESS_RESTART_THRESHOLDS["errors"])
    print(f"{'✅' if ok_err_rs else '❌'} Action errors: {rs['errors']} (target ≤ {STRESS_RESTART_THRESHOLDS['errors']})")
    rs_ok &= ok_err_rs
    ok_ret_rs = (rs_retries <= STRESS_RESTART_THRESHOLDS["retries_max"])
    print(f"{'✅' if ok_ret_rs else '❌'} Retry count: {rs_retries} (target ≤ {STRESS_RESTART_THRESHOLDS['retries_max']})")
    rs_ok &= ok_ret_rs
    ok_rc_rs = (rs_reconnects >= STRESS_RESTART_THRESHOLDS["min_reconnects"])
    print(f"{'✅' if ok_rc_rs else '❌'} Reconnects logged: {rs_reconnects} (target ≥ {STRESS_RESTART_THRESHOLDS['min_reconnects']})")
    rs_ok &= ok_rc_rs

    print("\n🎯 MPD-restart stress test KPIs met!" if rs_ok else "\n⚠️ MPD-restart stress test KPIs failed!")
