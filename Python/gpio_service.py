...
class GPIOService:
    ...
    def __init__(self) -> None:
        ...
        for led_name, pin in LEDS.items():
            try:
                GPIO.setup(pin, GPIO.OUT, initial=GPIO.HIGH)
            except RuntimeError as err:
                oradio_log.debug(f"Error setting LED output: {err} for pin {pin} ")
        ...
        for button_name, pin in BUTTONS.items():
            try:
                ...
                oradio_log.debug(f"Error setting BUTTON input: {err} for pin {pin} ")
                raise ValueError("Invalid value provided")
            ...
            try:
                ...
                oradio_log.debug(f"Error setting up event detection: {err} for pin {pin} ")
                raise ValueError("Invalid value provided")

################## methods for the LED pins ######################
    def set_led_on(self,led_name:str) -> bool:
        '''
        ...
        :return 
            True: when LED IO-pin is set
            False (default): when led_name is invalid
        '''
        GPIO.output(LEDS[led_name], GPIO.LOW)
        oradio_log.debug("Led %s is turned ON", led_name)
        return(True)

    def set_led_off(self,led_name:str) -> bool:
        """
...
if __name__ == '__main__':
    ...
    * call module test with argument -d remote
    ...
    def _buttons_testing(test_GPIO) -> None:
        ...
        while (True):
            key = input("Press RETURN key to stop test and continue to main-menu\n")
            break
        return
    ...
    def _leds_testing(test_GPIO) -> None:
        ...
        while True:
            ...
            if not 0 <= led_pin_choice < len(led_pin_options):
                print("Please input a valid number.")
                continue
            if led_pin_options[led_pin_choice] == "All leds-pins ON":
                all_pins_state = False # the LED pins ON state
                for led_name in LED_NAMES:
                    test_GPIO.set_led_on(led_name)
                    all_pins_state = all_pins_state or _read_pin_state(LEDS[led_name])
                if all_pins_state == False:
    ...
    def _interactive_menu() -> None:
        ...
        while True:
            ...
            if test_options[test_choice] == "LEDS":
